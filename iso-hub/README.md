...
- 仓库内另附两个完整四容器 compose 文件：`docker-compose.acr.yml`（iso-hub 走阿里云，**腾讯云线上在用**）与 `docker-compose.dockerhub.yml`（四容器均从 Docker Hub 拉取，海外服务器用），可直接 `docker compose -f <文件> up -d`。

### 1.2 阿里云 ACR 源部署方式（国内推荐）

> **✅ 2026-09-08 状态：** 阿里云 ACR 仓库 `dcchen/isohub` 已重建并恢复，当前标签 `latest` + `1.1.0`，已设为**公开**（无需 `docker login`）。腾讯云线上部署已切回本方式（用 `docker-compose.acr.yml`）。
