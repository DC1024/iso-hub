...
| 🔗 自定义源 | 「自定义源」页可添加官方/第三方镜像站的**任意直链**或**发行版源**（strategy 目录抓取），独立保存、不会被元数据刷新覆盖；发行版源支持**手动/自动定时刷新**，且刷新在后台子进程执行、绝不阻塞页面请求 |
...
新建目录，把下面内容保存为 `docker-compose.yml`（含 SMB/WebDAV/种子下载，四个容器）：

```yaml
services:
  iso-hub:
    # 国内阿里源, 无需 docker login
...
需要网络共享（SMB/WebDAV）或想改代码时用这种方式，会构建 iso-hub 主服务 + samba + webdav **三个容器**（种子下载的 qBittorrent 走现成镜像，无需构建，需要时补一段 [上面的 qbittorrent 容器配置](#1-使用-docker-部署) 到 compose 即可）。
...
## 自定义源 (官方/镜像站任意直链 + 发行版源)

「自定义源」页可添加清单里没有的发行版或版本（如 Kali、特定 LTS、官方地址等）：

- 支持两种条目：
  - **普通直链**：只需填**发行版名 + ISO 直链**，可选校验文件地址，直接合入镜像列表。
  - **发行版源（strategy 目录抓取）**：与官方源同机制的 `dated_directory` / `flat_listing` / `versioned_flat_listing` / `static` 策略，按 `listing_url` 目录**展开为多个具体 ISO 条目**。
...
- **请求线程只读缓存**：`/api/distros` 等请求只读取 `data/custom_repo_cache.json` 里的展开结果，**绝不在请求线程内发起网络请求**，从根上避免 waitress 线程被慢速 TLS 握手占满导致全站卡死。
- **后台 runner**：新增 `web/custom_repo_refresh.py`，以子进程方式读取 `custom_sources.json` 中所有 strategy 源并展开、写回缓存文件（进程内缓存按文件 mtime 失效后自动重读）。
- **两种触发方式**：
  - **手动刷新**：自定义源面板「🔄 刷新发行版源」按钮（`POST /api/custom-sources/refresh`）。
  - **自动定时刷新**：`GET/POST /api/custom-sources/auto-refresh` 查询/设置开关与间隔。**默认关闭**，可在设置或面板开启；间隔默认 `86400` 秒（每天），最小 `60` 秒。
- **刷新失败不丢旧结果**：单个源展开失败时保留缓存里的旧条目，避免已展示列表被清空。

> 因此：strategy 类型的发行版源在首次手动刷新前，镜像列表里不会立即出现其条目（尚未写入缓存）——先在面板触发一次刷新即可。普通直链不受此影响，添加后立即可见。
